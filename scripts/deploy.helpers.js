...
exports.minter = async (proxyAddress, starPrice, minId, maxId, mintingLimit, silent = false) => {
    _log(silent, 'loading contract', nftContractName);
    const minterContract = await ethers.getContractFactory(minterContractName);
    _log(silent, 'deploying implementation contract');
    let minterContractTx = await minterContract.deploy(proxyAddress, starPrice, minId, maxId, mintingLimit);
